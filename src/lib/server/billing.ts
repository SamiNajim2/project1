...
export const PLAN = { name: "Finance Analyst Pro", amount: 20, currency: "USD", interval: "month" } as const;
...
export function getStripe(): Stripe {
  ...
  stripe ??= new Stripe(key, { appInfo: { name: "Finance Analyst" } });
  return stripe;
}
...
/**
 * Access requires an active subscription to this app's price. Other products on the same Stripe
 * account (for example another app sharing the account) never unlock it.
 */
export function isActive(row: SubscriptionRow | null): boolean {
  if (!row?.status || !ACTIVE_STATUSES.has(row.status)) return false;
  const price = process.env.STRIPE_PRICE_ID;
  return !price || row.price_id === price;
}

function isOurs(subscription: Stripe.Subscription): boolean {
  const price = process.env.STRIPE_PRICE_ID;
  return !price || subscription.items.data.some((i) => i.price.id === price);
}
...
export async function syncSubscription(subscription: Stripe.Subscription, userIdHint?: string | null) {
  ...
  if (!isOurs(subscription)) return; // a subscription to another product
  ...
  const item = subscription.items.data.find((i) => i.price.id === process.env.STRIPE_PRICE_ID) ?? subscription.items.data[0];
  await upsertRow({
    user_id: userId,
...
export async function refreshSubscription(userId: string): Promise<SubscriptionRow | null> {
  ...
  let subscription: Stripe.Subscription | undefined = row.stripe_subscription_id
    ...
    : undefined;
  if (!subscription || !ACTIVE_STATUSES.has(subscription.status) || !isOurs(subscription)) {
    // The customer may have subscribed again, or the checkout return never reached us.
    const { data } = await stripe.subscriptions.list({ customer: row.stripe_customer_id, status: "all", limit: 20 });
    const ours = data.filter(isOurs);
    subscription = ours.find((s) => ACTIVE_STATUSES.has(s.status)) ?? ours[0];
  }
